...
using FastReflection.Properties;
    ...
    class Serializer
    {
        ...
        SerializedObject SerializeInternal(string name, object instance, Type declaredType)
        {
            ...
            if (instance == null || IsAtomic(type))
                child = new SerializedAtom { Name = name, Value = instance };
                ...
                foreach (MemberInfo memberInfo in reflectionProvider.GetSerializableMembers(type))
                {
                    ...
                    if (!valueFound) 
                    {
                        if (memberAttr.Required) 
                            throw new InvalidOperationException(string.Format(Resources.MissingRequiredValue, name, type.Name));
                    }
                    else
                    {
